    ...
    for input_name, input_item in inputs.inputs.items():
        ...
        try:
            ...
            if media_type_predicates:
                filter_block["clauses"].append({
                    "type": "or",
                    "predicates": media_type_predicates
                })

            if direction_predicates:
                filter_block["clauses"].append({
                    "type": "or",
                    "predicates": direction_predicates
                })
            ...
            response = client.post(
                "AnalyticsApi",  # API name
                "post_analytics_actions_aggregates_query",  # Method name
                "ActionAggregationQuery",  # Model name
                body  # Request body
            )
                ...
                for event in to_process_data:
                    try:
                        ...
                        for data_entry in event.get("data", []):
                            ...
                            if interval_str:
                                interval_start_time = datetime.strptime(
                                    interval_str.split("/")[0], "%Y-%m-%dT%H:%M:%S.%fZ"
                                ).timestamp()
                            else:
                                interval_start_time = round(start_time.timestamp(), 3)
