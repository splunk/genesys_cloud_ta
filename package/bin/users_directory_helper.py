...
FIELDS_TO_INDEX = [
    "id", "name", "email", "department", "state", "division", "version"
]
...
def stream_events(inputs: smi.InputDefinition, event_writer: smi.EventWriter):
    for input_name, input_item in inputs.inputs.items():
        ...
        try:
            ...
            logger.info("Fetching users directory snapshot")
            users = client.get("UsersApi", "get_users")

            event_counter = 0
            for user_obj in users:
                user_dict = user_obj.to_dict() if hasattr(user_obj, "to_dict") else user_obj
                record = {}
            ...
            logger.debug(f"Indexed {event_counter} user directory records")
            log.events_ingested(
                logger,
                input_name,
                SOURCETYPE,
